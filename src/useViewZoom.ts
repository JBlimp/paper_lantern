...
export function useViewZoom(view: RefObject<HTMLDivElement | null>) {
  ...
  function change(next: number, clientX?: number, clientY?: number) {
    ...
    if (!root) return;
    next = Math.round(Math.max(0.5, Math.min(3, next)) * 100) / 100;
    if (next === current.current) return;
    const bounds = root.getBoundingClientRect();
    const x = clientX ?? bounds.left + root.clientWidth / 2,
      y = clientY ?? bounds.top + Math.min(120, root.clientHeight / 2);
    const element =
      [...root.querySelectorAll<HTMLElement>('.pdf-page-slot, .sentence')].find((e) => {
    ...
    anchor.current = {
      element,
      fraction: rect ? (y - rect.top) / rect.height : 0,
      x: x - bounds.left,
      y: y - bounds.top,
  ...
  useEffect(() => {
    ...
    const wheel = (event: WheelEvent) => {
      ...
      const delta =
        event.deltaY * (event.deltaMode === 1 ? 16 : event.deltaMode === 2 ? root.clientHeight : 1);
      change(
        current.current * Math.exp(-Math.max(-100, Math.min(100, delta)) * 0.002),
        event.clientX,
        event.clientY,
      );
    };
