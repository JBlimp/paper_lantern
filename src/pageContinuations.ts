...
function heading(text: string) {
  return /^(?:references|bibliography|acknowledg(?:e)?ments|appendix|abstract|keywords)\b/i.test(text)
...
export function canContinue(left: Sentence, right: Sentence, before: Page, after: Page) {
  const a = left.text.trim(), b = right.text.trim();
  ...
  const aw = boundaryWords(left, before), bw = boundaryWords(right, after);
  if (!aw.length || !bw.length) return false;
  const end = aw.reduce((a, b) => a.y > b.y ? a : b), start = bw.reduce((a, b) => a.y < b.y ? a : b);
  if (end.y + end.height < before.height * .60 || start.y > after.height * .70) return false;
  if (Math.min(end.height, start.height) / Math.max(end.height, start.height) < .8) return false;
...
export function linkPageContinuations(input: Page[]): Sentence[][] {
  const pages = input.map(p => ({ ...p, sentences: p.sentences.map(s => ({ ...s, fragments: s.fragments.map(f => ({ ...f })) })) }));
  ...
  for (let i = 0; i < pages.length - 1; i++) {
    const before = pages[i], after = pages[i + 1];
    const left = before.sentences.at(-1), right = after.sentences[0];
    if (!left || !right || !canContinue(left, right, before, after)) continue;
    const text = /[a-z][-\u00ad]$/.test(left.text) && /^[a-z]/.test(right.text)
      ? left.text.slice(0, -1) + right.text : left.text + ' ' + right.text;
    ...
    for (const member of members) { member.id = left.id; member.text = text; member.pages = numbers; }
