...
type Props = {
  ...
  active: string | null;
  jump: { page: number; serial: number };
  onPage(page: number): void;
  onReady(sentences: Sentence[], page: number): void;
...
function PageSlot({ number, near, ...props }: Props & { number: number; near: boolean }) {
  ...
  const ready = useCallback(
    (sentences: Sentence[]) => props.onReady(sentences, number),
    [props.onReady, number],
  );
  ...
  return (
    ...
    >
      {near ? (
        ...
        <div className="page-placeholder">{number}</div>
      )}
    </div>
  );
}
export function ContinuousPdf(props: Props) {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const target = root.getBoundingClientRect().top + Math.min(120, root.clientHeight * 0.25);
      ...
      const slot = atBottom
        ? slots.at(-1)
        : (slots.find((s) => s.getBoundingClientRect().bottom > target) ?? slots.at(-1));
      if (slot) {
        const number = Number(slot.dataset.page);
        if (current.current !== number) {
          current.current = number;
          latest.current.onPage(number);
        }
      }
    };
    const schedule = () => {
  ...
  useEffect(() => {
    const root = scroll.current!,
      slot = root.querySelector<HTMLElement>(`[data-page="${props.jump.page}"]`);
    if (!slot) return;
    ...
    <>
      <div className="pane-heading">
        ...
        <button
          ...
          onClick={reset}
          title="Ctrl+휠로 원문 확대·축소 · 클릭하면 100%"
          aria-label="원문 배율 초기화"
        >
          {Math.round(zoom * 100)}%
        </button>
      </div>
      <div className="pdf-scroll" ref={scroll}>
        <div className="pdf-pages" style={{ width: `${zoom * 100}%` }}>
          {Array.from({ length: props.doc.numPages }, (_, index) => (
            <PageSlot key={index + 1} {...props} number={index + 1} near={near.has(index + 1)} />
          ))}
        </div>
      </div>
    </>
