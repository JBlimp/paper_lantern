...
export function readPrompts(): Prompts {
  try {
    ...
    return Object.fromEntries(Object.entries(defaultPrompts).map(([key, fallback]) => [key, typeof saved[key] === 'string' && saved[key].length <= (key.endsWith('Model') ? 100 : 8000) ? saved[key] : fallback])) as Prompts;
