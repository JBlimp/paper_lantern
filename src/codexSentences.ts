...
export async function translateDocument(
  units: SourceUnit[],
  pageCount: number,
  request: (pages: { page: number; text: string }[]) => Promise<PaperTranslation>,
  signal: AbortSignal,
  ...
  const result = await request(documentPages); // Exactly one model request for the entire paper.
  signal.throwIfAborted();
  ...
  return { pages, translations, work: { complete: true } };
}
