...
export function referenceRows(words: Word[], width: number, height: number): string[] {
  const items = words.filter(
    (w) => w.str.trim() && Math.abs(w.angle ?? 0) < 0.1 && w.y > height * 0.04 && w.y < height * 0.95,
  );
  const right = items.filter((w) => w.x > width * 0.48 && w.x < width * 0.65).length;
  const left = items.filter((w) => w.x < width * 0.45).length;
  const columns =
    right > 8 && left > 8
      ? [items.filter((w) => w.x < width * 0.49), items.filter((w) => w.x >= width * 0.49)]
      : [items];
  return columns.flatMap((column) => {
    const rows: { y: number; size: number; words: Word[] }[] = [];
    for (const word of [...column].sort((a, b) => a.y - b.y || a.x - b.x)) {
      const row = rows.findLast((r) => Math.abs(r.y - word.y) < Math.max(r.size, word.height) * 0.45);
      if (row) row.words.push(word);
      else rows.push({ y: word.y, size: word.height, words: [word] });
    }
    return rows.map((row) =>
      row.words
        .sort((a, b) => a.x - b.x)
...
export function parseReferences(pages: { page: number; rows: string[] }[]): Map<number, Reference> {
  const result = new Map<number, Reference>(),
    ambiguous = new Set<number>();
  let inReferences = false,
    current: Reference | null = null;
  const flush = () => {
  ...
  for (const page of pages)
    for (const row of page.rows) {
      if (/^(?:\d+[. ]+)?(?:references|bibliography|참고\s*문헌)\s*$/i.test(row)) {
        inReferences = true;
        continue;
      }
      if (!inReferences) continue;
      if (/^(?:appendix\b|supplementary\s+(?:material|information))/i.test(row)) {
        flush();
        inReferences = false;
        continue;
      }
      ...
      if (start) {
        flush();
        current = { number: Number(start[1] ?? start[2]), text: start[3], page: page.page };
  ...
  flush();
  ambiguous.forEach((n) => result.delete(n));
  return result;
}
