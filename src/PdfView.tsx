...
import { readPage, observeSentences } from './readPage';
...
export function PdfView({
  ...
  useEffect(() => observeSentences(doc, () => setSentenceRevision((n) => n + 1)), [doc]);
  ...
  const [width, setWidth] = useState(650),
    [height, setHeight] = useState(850),
    [boxes, setBoxes] = useState<Box[]>([]);
  useEffect(() => {
    let timer: ReturnType<typeof setTimeout>;
    const observer = new ResizeObserver(([entry]) => {
      clearTimeout(timer);
      timer = setTimeout(() => setWidth(Math.max(80, Math.floor(entry.contentRect.width))), 100);
    });
    ...
    const task = async () => {
      ...
      for (const sentence of sentences)
        for (const fragment of sentence.fragments) {
          ...
          range.setStart(node, fragment.start);
          range.setEnd(node, fragment.end);
      ...
      const positions = nodes.map((node) => {
        ...
        return { start, end: joined.length - 1 };
      });
      ...
      for (const match of joined.matchAll(/\[\s*\d{1,4}(?:\s*[,;–-]\s*\d{1,4})*\s*\]/g)) {
        ...
        positions.forEach((position, index) => {
          ...
          if (!node || position.end <= start || position.start >= end) return;
          const range = document.createRange();
          range.setStart(node, Math.max(0, start - position.start));
          range.setEnd(node, Math.min(position.end, end) - position.start);
          for (const r of range.getClientRects())
            if (r.width > 0)
              links.push({
                numbers,
  ...
  }, [doc, page, width, mode, onReady, onError, onSize, sentenceRevision]);
  return (
    <div ref={host} className="pdf-host">
      <div
        ...
        onMouseMove={(event) => {
          ...
          onActive(
            boxes.find((b) => x >= b.x && x <= b.x + b.width && y >= b.y && y <= b.y + b.height)?.id ?? null,
          );
        }}
      >
        ...
        <div className="citation-links">
          {citations.map((link, i) => (
            <button
              ...
              aria-label={`인용 논문 ${link.numbers.join(', ')}`}
              title={`참고문헌 [${link.numbers.join(', ')}] 보기`}
              onClick={(e) => {
                e.stopPropagation();
                setSelectedCitation(link.numbers);
              }}
