...
export function graphicRegions(
  ...
  for (const shape of shapes) {
    ...
    for (let i = groups.length - 1; i >= 0; i--) {
      const g = groups[i],
        pad = 5;
      if (
        joined.x > g.x + g.width + pad ||
        joined.x + joined.width < g.x - pad ||
        joined.y > g.y + g.height + pad ||
        joined.y + joined.height < g.y - pad
      )
        continue;
      const x = Math.min(joined.x, g.x),
        y = Math.min(joined.y, g.y);
      joined = {
        x,
        y,
        width: Math.max(joined.x + joined.width, g.x + g.width) - x,
        height: Math.max(joined.y + joined.height, g.y + g.height) - y,
        count: joined.count + g.count,
        image: joined.image || g.image,
      };
      groups.splice(i, 1);
      i = groups.length;
    }
  ...
  return groups.filter(
    (g) =>
      ...
      g.width >= 45 &&
      g.height >= 28 &&
      g.width * g.height < width * height * 0.65,
  );
}
