...
export function textRows(words: Word[]): Row[] {
  ...
  for (const word of words.filter((w) => w.str.trim()).sort((a, b) => a.y - b.y || a.x - b.x)) {
    let band = bands.findLast(
      (b) =>
        Math.abs((b[0].baseline ?? b[0].y + b[0].height) - (word.baseline ?? word.y + word.height)) <
        Math.max(b[0].height, word.height) * 0.45,
    );
    if (!band) {
      band = [];
      bands.push(band);
  ...
  for (const band of bands) {
    ...
    const flush = () => {
      ...
      const x = Math.min(...group.map((w) => w.x)),
        y = Math.min(...group.map((w) => w.y));
      rows.push({
        words: group,
        text: group
          .map((w) => w.str)
        ...
        x,
        y,
        width: Math.max(...group.map((w) => w.x + w.width)) - x,
        height: median(group.map((w) => w.height)),
        font: group[0].font,
      });
      group = [];
    };
...
function marginKey(row: Row, page: PageText) {
  ...
  const band = row.y < page.height * 0.12 ? 'top' : row.y > page.height * 0.86 ? 'bottom' : '';
  if (!band) return '';
...
export function buildBodyProfile(pages: PageText[]): BodyProfile {
  const weights = new Map<number, number>(),
    margins = new Map<string, Set<number>>();
  pages.forEach((page, index) => {
    const rows = textRows(page.words.filter((w) => Math.abs(w.angle ?? 0) < 0.1));
  ...
  return {
    bodyHeight,
    repeatedMargins: new Set(
      [...margins]
        .filter(([, ids]) => ids.size >= Math.max(2, Math.ceil(pages.length * 0.3)))
        .map(([key]) => key),
    ),
  };
}
...
function isCaption(row: Row, graphics: Rect[], bodyHeight: number) {
  const match = /^(?:fig(?:ure)?\.?|table|plate|scheme|그림|표)\s*(?:\d+[a-z]?|[IVX]+)\b\s*(.*)$/i.exec(
    row.text,
  );
  if (!match) return false;
  // A paragraph discussing a figure is still body text.
  if (
    /^(?:shows?|illustrates?|demonstrates?|presents?|compares?|summari[sz]es?|reports?|depicts?|provides?|is|was|contains?|lists?)\b/i.test(
      match[1],
    )
  )
    return false;
  return (
    /^[.:\-–—(]/.test(match[1]) ||
    row.height < bodyHeight * 0.95 ||
    graphics.some((g) => intersects(g, row, bodyHeight * 4))
  );
}
export function filterBody(
  page: PageText,
  context: BodyContext = {},
): { words: Word[]; excluded: Map<number, string> } {
  ...
  for (const word of page.words) {
    if (Math.abs(word.angle ?? 0) > 0.1) excluded.set(word.index, 'rotated annotation');
  ...
  for (const row of rows) {
    const margin = Number.isFinite(page.height) && (row.y < page.height * 0.12 || row.y > page.height * 0.86);
    if (
      margin &&
      (row.y < page.height * 0.1 || row.y > page.height * 0.92) &&
      row.height <= bodyHeight * 1.1 &&
      profile.repeatedMargins.has(marginKey(row, page))
    )
      exclude(row.words, 'repeated header/footer');
    if (
      margin &&
      !rows.some(
        (r) =>
          r !== row &&
          /[a-z]/i.test(r.text) &&
          row.y <= r.y + r.height * 0.3 &&
          r.y - row.y < bodyHeight &&
          row.x >= r.x - bodyHeight &&
          row.x <= r.x + r.width + bodyHeight,
      ) &&
      /^(?:[-–—]?\s*\d+\s*[-–—]?|page\s+\d+(?:\s+of\s+\d+)?)$/i.test(row.text)
    )
      exclude(row.words, 'page number');
    if (
      margin &&
      /^(?:arxiv\s*:|preprint(?:\s+submitted)?\b|©|copyright\s+\d|https?:\/\/arxiv\.)/i.test(row.text)
    )
      exclude(row.words, 'margin metadata');
  }
  ...
  for (const graphic of graphics) {
    if (graphic.width * graphic.height > page.width * page.height * 0.65) continue;
    const enclosedProse = rows.filter(
      (row) =>
        row.text.length > 60 &&
        row.height >= bodyHeight * 0.95 &&
        intersects(graphic, { x: row.x + row.width / 2, y: row.y + row.height / 2, width: 0, height: 0 }),
    );
    const hasCaption = rows.some(
      (row) => isCaption(row, graphics, bodyHeight) && intersects(graphic, row, bodyHeight * 5),
    );
    // A bordered theorem or shaded prose callout is not automatically a figure.
    if (
      !hasCaption &&
      enclosedProse.length >= 2 &&
      enclosedProse.reduce((n, row) => n + row.text.length, 0) >= 160
    )
      continue;
    for (const row of rows) {
      ...
      if (intersects(graphic, center)) exclude(row.words, 'inside figure/table');
        ...
        row.text.length < 45 &&
        row.width < graphic.width * 0.8 &&
        intersects(graphic, row, bodyHeight * 1.7)
      )
        exclude(row.words, 'figure label');
    }
  }
  ...
  for (const caption of captions) {
    ...
    for (const row of rows.filter((r) => r.y > caption.y && Math.abs(r.x - caption.x) < bodyHeight * 2)) {
      ...
      if (
        gap > end.height * 1.7 ||
        Math.abs(row.height - caption.height) > bodyHeight * 0.12 ||
        isCaption(row, graphics, bodyHeight)
      )
        break;
      if (/[.!?]\s*$/.test(end.text) && gap > end.height * 1.3 && /^[A-Z]/.test(row.text)) break;
      exclude(row.words, 'caption continuation');
      end = row;
    }
    ...
    const preceding = rows
      .filter(
        (r) =>
          r.y < caption.y &&
          r.x >= caption.x - bodyHeight * 2 &&
          r.x < caption.x + Math.max(caption.width, page.width * 0.35),
      )
      .sort((a, b) => b.y - a.y);
    for (const row of preceding) {
      if (caption.y - row.y > page.height * 0.32 || row.text.length > 65 || row.height >= bodyHeight * 0.98)
        break;
