import type { SelectionChat } from './FloatingChats';
import type { CodexWork } from './codexSentences';
...
export type SavedSession = {
  ...
  page: number;
  codexSentences?: Sentence[][];
  codexWork?: CodexWork;
  translations: Record<string, string>;
  completedPages: number;
  selectionChats?: SelectionChat[];
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      if (
        !session ||
        session.schema !== 1 ||
        !['chrome', 'codex'].includes(session.provider) ||
        !['auto', 'single', 'double'].includes(session.mode) ||
