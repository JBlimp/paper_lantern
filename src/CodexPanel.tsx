  ...
  useEffect(() => {
    const change = () => {
      ...
      if (node?.closest('.reader') && selected?.toString().trim()) {
        ...
        selectedPages.current = selectedPdfPages();
      }
    };
  ...
  async function ask() {
    ...
    try {
      ...
      if (scope === 'selection') {
        ...
        pages.push(...(selectedPages.current.length ? selectedPages.current : [{ page: selectionPage, text: selection }]));
      } else {
      ...
      request.signal.throwIfAborted();
      const result = await client.request<{ answer: string; pages: number[] }>('ask', { question: text, history, pages: questionContext(pages, text + ' ' + history.filter(h => h.role === 'user').map(h => h.text).join(' '), page), model, systemPrompt }, request.signal);
