...
function linesOf(words: Word[]): Line[] {
  ...
  for (const word of [...words].sort((a, b) => a.y - b.y || a.x - b.x)) {
    ...
    let line = lines.findLast(
      (l) =>
        Math.abs(l.baseline - baseline) < Math.max(l.height, word.height) * 0.3 ||
        (Math.min(l.height, word.height) < Math.max(l.height, word.height) * 0.8 &&
          Math.abs(l.baseline - baseline) < Math.max(l.height, word.height) * 0.7),
    );
    if (!line) {
      line = { words: [], x: word.x, y: word.y, height: word.height, right: 0, baseline };
      lines.push(line);
    }
...
export function extractSentences(
  ...
  const words = filterBody({ words: input, width: pageWidth, height: pageHeight }, context).words.filter(
    (w) => w.str.trim(),
  );
  ...
  for (const line of raw)
    for (let i = 1; i < line.words.length; i++) {
      ...
      if (
        right.x - left.x - left.width > Math.max(18, line.height * 1.8) &&
        center > pageWidth * 0.35 &&
        center < pageWidth * 0.65
      )
        gaps.push(center);
    }
  const two = mode === 'double' || (mode === 'auto' && gaps.length >= 3);
  const gutter =
    mode === 'double' || !gaps.length
      ? pageWidth / 2
  ...
  if (two) {
    ...
    for (const heading of [...spanning, { y: Infinity } as Line]) {
      const band = words.filter(
        (w) => w.y > floor && w.y < heading.y - 1 && !spanning.some((l) => l.words.includes(w)),
      );
      ordered.push(
        ...linesOf(band.filter((w) => w.x + w.width / 2 < gutter)),
        ...linesOf(band.filter((w) => w.x + w.width / 2 >= gutter)),
      );
  ...
  for (const line of ordered) {
    ...
    const continues =
      ...
      !/[.!?:][\s\])}"']*$/.test(previousText) &&
      /^[a-z(\[]/.test(nextText) &&
      Math.min(prev.height, line.height) / Math.max(prev.height, line.height) >= 0.8;
    const columnChange = !!prev && (line.y < prev.y || (two && Math.abs(line.x - prev.x) > pageWidth * 0.25));
    if (
      !prev ||
      (!continues && columnChange) ||
      (!columnChange && line.y - prev.y > Math.max(prev.height, line.height) * (continues ? 3 : 1.85)) ||
      Math.max(line.height, prev.height) / Math.min(line.height, prev.height) > 1.3
    )
  ...
  for (const paragraph of paragraphs) {
    ...
    for (const line of paragraph) {
      for (const [wi, word] of line.words.entries()) {
        ...
        const adjacent = previousWord && gap <= Math.min(previousWord.height, word.height) * 0.12;
        ...
        for (let offset = 0; offset < word.str.length; offset++) {
          ...
          if (char === '\u200b') continue;
          if (char === '\u00ad') {
            if (offset === word.str.length - 1) {
              text += '-';
              refs.push({ item: word.index, offset });
            }
            continue;
          }
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      while (
        i + 1 < segments.length &&
        /\b(?:Fig|Figs|Eq|Eqs|Dr|Prof|Sec|Secs|Ref|Refs|No|Vol|pp|vs|al|cf|e\.g|i\.e|[A-Z])\.\s*$/.test(chunk)
      )
        chunk += segments[++i].segment;
