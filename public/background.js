...
chrome.action.onClicked.addListener(async (tab) => {
  if (tab.id && /^(https?|file):/.test(tab.url || '')) {
    try {
      ...
      /* Restricted pages use the extension's opening instructions. */
    }
  }
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  (async () => {
    if (
      ...
      !/^(https?|file):/.test(sender.tab.url || '')
    )
      throw new Error('원본 PDF 탭에서 실행해 주세요.');
    ...
    if (Number(response.headers.get('content-length')) > limit)
      throw new Error('100MB 이하의 PDF를 열어 주세요.');
    ...
    for (;;) {
      ...
      if (signature.length < 1024)
        signature += String.fromCharCode(...value.subarray(0, 1024 - signature.length));
      ...
      for (let offset = 0; offset < value.length; offset += 32768)
        send({ chunk: btoa(String.fromCharCode(...value.subarray(offset, offset + 32768))) });
    }
    if (!signature.includes('%PDF-'))
      throw new Error('현재 주소에서 PDF 대신 다른 응답을 받았습니다. 로그인이나 파일 주소를 확인해 주세요.');
    let name = 'paper.pdf';
    try {
      ...
      send({
        error:
          error.name === 'AbortError'
            ? 'PDF 읽기 시간이 초과되었습니다. 탭을 새로고침해 주세요.'
            : `${error.message} 로컬 PDF라면 확장 설정에서 파일 URL 액세스를 허용해 주세요.`,
      });
    })
    .finally(() => {
      clearTimeout(timer);
