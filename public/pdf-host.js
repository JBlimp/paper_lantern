(() => {
  if (window.top !== window || document.contentType !== 'application/pdf' || document.getElementById('paper-lantern-reader')) return;
  ...
  frame.style.cssText = 'position:fixed!important;inset:0!important;width:100%!important;height:100%!important;border:0!important;z-index:2147483646!important;background:white!important;color-scheme:light!important;';
  document.documentElement.append(frame);

  const host = document.createElement('div');
  host.id = 'paper-lantern-toggle';
  host.style.cssText = 'all:initial!important;position:fixed!important;left:18px!important;bottom:18px!important;z-index:2147483647!important;display:block!important;';
  const root = host.attachShadow({ mode: 'open' });
  const button = document.createElement('button');
  button.type = 'button';
  button.setAttribute('role', 'switch');
  button.setAttribute('aria-label', 'Paper Lantern 리더');
  button.style.cssText = 'all:initial;box-sizing:border-box;display:block;width:46px;height:28px;padding:3px;border:1px solid #ffffffaa;border-radius:20px;cursor:pointer;box-shadow:0 2px 8px #0004;';
  const knob = document.createElement('span');
  knob.setAttribute('aria-hidden', 'true');
  knob.style.cssText = 'display:block;width:20px;height:20px;border-radius:50%;background:#fff;box-shadow:0 1px 3px #0003;';
  button.append(knob); root.append(button); document.documentElement.append(host);
  let enabled = true;
  function update() {
