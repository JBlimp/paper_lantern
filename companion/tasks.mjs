...
export function translationTask(params) {
  ...
  return {
    prompt: 'Translate targets into fluent Korean using the page context. Preserve all meaning and numbers. Keep technical terms, algorithms, named methods, acronyms and mathematical notation in original English. Return exactly one translation per target with its unchanged id. Never merge, omit or add targets. Context is for interpretation only. Data:\n' + JSON.stringify({ context, targets: sentences }),
