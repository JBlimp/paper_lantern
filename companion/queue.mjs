export class RequestQueue {
  ...
  acquire(signal) {
    ...
    return new Promise((resolve, reject) => {
      const entry = { signal, resolve, reject, cancel: () => {
  ...
  drain() {
    while (this.active < this.limit && this.waiting.length) {
      const entry = this.waiting.shift(); entry.signal.removeEventListener('abort', entry.cancel);
      if (entry.signal.aborted) { entry.reject(new Error('요청이 중단되었습니다.')); continue; }
      this.active++;
      let released = false;
