...
function Initialize-AppController {
  ...
  $script:appTray.Text = 'Paper Lantern v0.6.0'
  ...
  $item = $appMenu.Items.Add('상태 보기'); $item.add_Click({
    ...
    [Windows.Forms.MessageBox]::Show($form, ('Paper Lantern v0.6.0' + "`r`n" + $connectionLabel.Text + "`r`n설치: " + $PSScriptRoot), 'Paper Lantern', 'OK', 'Information') | Out-Null
  })
