...
try { const page=await browser.newPage();await page.clock.install();
 await page.addInitScript(()=>{window.__protocol=8;window.__allow=false;window.__connections=0;window.chrome??={};window.chrome.runtime={id:'test',connect(){window.__connections++;const messages=[],disconnects=[];let closed=false;return{onMessage:{addListener(f){messages.push(f);}},onDisconnect:{addListener(f){disconnects.push(f);}},disconnect(){if(!closed){closed=true;disconnects.forEach(f=>f());}},postMessage(m){if(window.__silent)return;setTimeout(()=>{if(!closed)messages.forEach(f=>f(window.__allow?{id:m.id,result:{state:"ready",protocolVersion:window.__protocol,models:[{id:'test',name:'Test',isDefault:true}]}}:{id:m.id,error:'Not registered yet'}));},10);}};}};});
 ...
 await expect(page.locator('.connection-status strong')).toContainText('연결됨');
 await page.evaluate(()=>window.__protocol=7);
 await page.getByRole('button',{name:'모델 목록 다시 불러오기'}).click();await page.clock.runFor(100);
 await expect(page.locator('.connection-status strong')).toContainText('연결 안 됨');
 await expect(page.locator('.connection-status')).toContainText('버전이 너무 오래되었습니다');
 await page.evaluate(()=>window.__protocol=8);
 await page.getByRole('button',{name:'모델 목록 다시 불러오기'}).click();await page.clock.runFor(100);
 await expect(page.locator('.connection-status strong')).toContainText('연결됨');
