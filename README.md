...
- **드래그해서 질문** — PDF 텍스트를 드래그하고 **Codex에게 질문**을 누르면 선택한 문장과 페이지를 질문창에 첨부합니다.
...
번역·질문의 기본 모델은 각각 설정할 수 있으며, 지정하지 않으면 Codex 계정의 기본 모델을 사용합니다. 모델·프롬프트 변경은 다음 요청부터 적용되고 이미 완료된 번역은 유지됩니다. 전체를 다시 번역하려면 번역 엔진을 Chrome으로 바꿨다가 Codex로 다시 선택하세요.

Chrome 번역은 로컬 모델, Codex는 로그인한 계정의 온라인 모델과 사용량을 이용합니다. Codex 요청은 ephemeral thread로 처리하고, 이어지는 질문에는 저장된 최근 대화와 논문 문맥을 전달합니다.

### 저장과 이어 읽기

번역·대화·마지막 페이지·패널 상태를 확장 프로그램의 IndexedDB에 자동 저장합니다. 원본 PDF 파일은 복사하지 않습니다. 동일한 파일을 다시 열면 이름이나 경로가 달라져도 파일 내용의 SHA-256 값으로 이전 세션을 찾습니다. 내용이 바뀐 파일은 새 세션으로 시작합니다.

완료된 번역은 모델을 다시 실행하지 않고 표시합니다. 중단된 번역은 저장된 문장을 건너뛰고 이어갑니다. 설정은 브라우저 로컬 저장소에 별도로 보관합니다. 저장 공간이 부족하거나 저장소를 사용할 수 없으면 알림을 표시하고 현재 탭에서 계속 읽을 수 있습니다. 다른 Chrome 프로필이나 컴퓨터로 자동 동기화되지는 않습니다.
