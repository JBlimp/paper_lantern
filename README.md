...
- **시스템 프롬프트 설정** — 상단 **설정**에서 번역용·질문용 프롬프트를 각각 편집하고 기본값으로 복원할 수 있습니다.
...
**v0.2.0에서 업데이트한다면 `companion/install.cmd`도 다시 실행하세요.** 새 시스템 프롬프트를 전달하려면 연결 프로그램 업데이트가 필요합니다.

연결은 탭별로 관리됩니다. **Codex 연결**을 누르면 로컬 연결 프로그램과 `codex app-server`가 시작되고, **연결 해제** 또는 PDF 탭 종료 시 함께 종료됩니다. 질문 패널을 닫거나 Chrome 번역으로 전환하는 것만으로는 연결이 종료되지 않습니다. 로컬 HTTP 서버나 포트는 사용하지 않습니다.

최초 모델은 Codex 계정의 기본 모델입니다. 질문 패널의 **모델 선택**은 번역과 질문에 공통으로 적용되고, Codex 번역을 선택하면 현재 선택한 모델이 상단에 표시됩니다. 모델·프롬프트 변경은 다음 요청부터 적용되며 이미 완료된 번역은 그대로 남습니다. 전체를 다시 번역하려면 번역 엔진을 Chrome으로 바꿨다가 Codex로 다시 선택하세요.

시스템 프롬프트 설정만 브라우저에 저장합니다. 번역·대화는 저장하지 않습니다. 번역의 문장 연결과 답변 페이지 형식은 유지하면서 언어, 문체, 전문 용어 처리 등을 설정할 수 있습니다.
